...
from mmseg.ops.token_learner import TokenFuser, Tokenlearner, TokenlearnerV11
...
class TokenLearnerTransformerEncoderLayer(BaseModule):
    ...
    def __init__(
        ...
        super(TokenLearnerTransformerEncoderLayer, self).__init__()
        ...
        self.token_learner = TokenlearnerV11(in_channels=embed_dims, num_tokens=tl_num_tokens)
        self.token_fuser = TokenFuser(in_channels=embed_dims, num_tokens=tl_num_tokens)
    ...
    def forward(self, x, H, W):
        ...
        x = torch.cat([x_origin[:, :1], x], dim=1)
        x = self.attn(self.norm1(x), identity=x)
        x = self.ffn(self.norm2(x), identity=x)
        cls_token = x[:, :1]
        x = self.token_fuser(x[:, 1:], x_origin[:, 1:].view(B, H, W, C))
        x = x.view(B, -1, C)
...
class TokenLearnerVisionTransformer(BaseModule):
    """Vision Transformer.
    ...
    def __init__(
        ...
        super(TokenLearnerVisionTransformer, self).__init__()
        ...
        for i in range(num_layers):
            if i < tl_layer:
                ...
                self.layers.append(
                    TokenLearnerTransformerEncoderLayer(
                        embed_dims=embed_dims,
                        num_heads=num_heads,
                        feedforward_channels=mlp_ratio * embed_dims,
                        attn_drop_rate=attn_drop_rate,
    ...
    def init_weights(self):
        if isinstance(self.pretrained, str):
            ...
            super(TokenLearnerVisionTransformer, self).init_weights()
    ...
    def _freeze_stages(self, frozen_stages=-1):
        ...
        for i in range(len(self.layers)):
            if i + 1 <= frozen_stages:
                for m in [self.layers[i].attn, self.layers[i].ffn, self.layers[i].norm1, self.layers[i].norm2]:
                    m.eval()
                    for param in m.parameters():
                        param.requires_grad = False
            else:
        ...
        if frozen_stages == len(self.layers) + 1:
            for i in range(self.tl_layer, len(self.layers)):
                for m in [self.layers[i].token_learner, self.layers[i].token_fuser]:
                    m.eval()
                    for param in m.parameters():
                        param.requires_grad = False
    ...
    def forward(self, inputs):
        ...
        for i, layer in enumerate(self.layers):
            layer_args = [x] if i < self.tl_layer else [x, *hw_shape]
    ...
    def train(self, mode=True):
        super(TokenLearnerVisionTransformer, self).train(mode)
