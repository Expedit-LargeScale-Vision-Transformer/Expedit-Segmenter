...
def softmax_with_policy(attn, policy, eps=1e-6):
    B, N, _ = policy.size()
    # _, N, N = attn.size()
    attn_policy = policy.reshape(B, 1, N)  # * policy.reshape(B, 1, N, 1)
...
def multi_head_attention_forward(
    ...
    #
    attn_output, attn_output_weights = _scaled_dot_product_attention(q, k, v, attn_mask, dropout_p, policy=policy)
...
class NNMultiheadAttention(Module):
    ...
    def forward(self, query: Tensor, key: Tensor, value: Tensor, key_padding_mask: Optional[Tensor] = None,
                ...
                policy=None,) -> Tuple[Tensor, Optional[Tensor]]:
        r"""
    Args:
...
class MultiheadAttention(BaseModule):
    ...
    def __init__(self,
        ...
        self.attn = NNMultiheadAttention(embed_dims, num_heads, attn_drop, **kwargs)
        if self.batch_first:
            ...
            def _bnc_to_nbc(forward):
                ...
                def forward_wrapper(**kwargs):
                    ...
                    attn_output, attn_weights = forward(**kwargs)
...
class TransformerEncoderLayer(BaseModule):
    ...
    def forward(self, x, policy=None):
        x = self.attn(self.norm1(x), identity=x, policy=policy)
        x = self.ffn(self.norm2(x), identity=x)
        return x


@BACKBONES.register_module()
class DynamicViT(VisionTransformer):
    """
    ...
    def build_token_reconstruction_block(self, cfg: dict):
        ...
        self.reconstruction_location = cfg.get("layer", -1)
        self.token_reconstruction_layer = TokenReconstructionBlock(embed_dim=self.embed_dims, cfg=cfg)

    def reconstruct(self, x, policy_list, non_cls_list):
        if self.training:
            B, N, C = x.shape
            out = x
            for i in reversed(range(len(policy_list))):
                policy = policy_list[i]
                non_cls = non_cls_list[i]
                if i > 0:
                    prev_policy = policy_list[i - 1]
                else:
                    prev_policy = torch.ones_like(policy)
                policy_compl = prev_policy - policy

                unpooler = self.unpooling.derive_unpooler()
                unpooler.update_state(feat_before_pooling = 
                    non_cls[policy_compl.expand(-1, -1, C).bool()].view(B, -1, C))
                unpooler.update_state(feat_after_pooling = 
                    non_cls[policy.expand(-1, -1, C).bool()].view(B, -1, C))
                out_non_cls = unpooler.call(
                    out[policy.expand(-1, -1, C).bool()].view(B, -1, C))
                out[policy_compl.expand(-1, -1, C).bool()] = out_non_cls[0].view(-1)
            return out
        else:
            out = x
            for i in reversed(range(len(policy_list))):
                ...
                unpooler = self.unpooling.derive_unpooler()
                unpooler.update_state(feat_before_pooling = torch.gather(
                    non_cls, dim=1, index=idx_compl.unsqueeze(-1).expand(-1, -1, C)))
                unpooler.update_state(feat_after_pooling = torch.gather(
                    non_cls, dim=1, index=idx.unsqueeze(-1).expand(-1, -1, C)))
                out_non_cls = unpooler.call(out)
                out_tmp = torch.empty_like(non_cls)
                out_tmp[0, idx[0]] = out
                out_tmp[0, idx_compl[0]] = out_non_cls[0]
                out = out_tmp
            return out

    def forward(self, inputs):
        B, _, H, W = inputs.shape
        # self.resize_shape = (320, 320)
        ...
        for i, layer in enumerate(self.layers):
            if i in self.pruning_loc:
                ...
                if self.training:
                    ...
                    policy = torch.cat([cls_policy, hard_keep_decision], dim=1)     # {0, 1}^(BxNx1)
                    non_cls_list.append(x[:, 1:])
                    policy_list.append(hard_keep_decision)
                    if self.use_checkpoint:
                        x = checkpoint(layer, x, policy)
                    else:
                        x = layer(x, policy)
                    ...
                    policy_list.append(keep_policy)
                    non_cls_list.append(x[:, 1:])
                ...
                layer_args = [x, policy] if self.training else [x]
                if self.use_checkpoint:
                    x = checkpoint(layer, *layer_args)
                else:
                    x = layer(*layer_args)
            ...
            if i in self.out_indices:
                ...
                if len(policy_list) > 0:
                    out = self.reconstruct(out, policy_list, non_cls_list)
                B, _, C = out.shape
