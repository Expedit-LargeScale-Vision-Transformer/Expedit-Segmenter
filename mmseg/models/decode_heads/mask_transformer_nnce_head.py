...
class MaskTransformerNNCEHead(BaseDecodeHead):
    def __init__(
        self,
        n_cls,
        topk_cls,
    ...
    def forward(self, x):
        ...
        cls_seg_feat = cls_seg_feat.view(self.n_cls, -1)[topk_index.view(-1)].view(B, self.topk_cls, -1)
...
@HEADS.register_module()
class MaskTransformerNNCE2Head(BaseDecodeHead):
    def __init__(
        self,
        n_cls,
        topk_cls,
    ...
    def forward(self, x):
        ...
        x = x.view(B, C, -1).permute(0, 2, 1)   
        cls_emb = self.cls_emb.view(self.n_cls, -1)[topk_index.view(-1)].view(B, self.topk_cls, -1)
        x = self.proj_dec(x)
        x = torch.cat((x, cls_emb), 1)
        ...
        for blk in self.blocks:
            x = blk(x)
        x = self.decoder_norm(x)
        patches, cls_seg_feat = x[:, : -self.topk_cls], x[:, -self.topk_cls :]
        patches = patches @ self.proj_patch
        cls_seg_feat = cls_seg_feat @ self.proj_classes
