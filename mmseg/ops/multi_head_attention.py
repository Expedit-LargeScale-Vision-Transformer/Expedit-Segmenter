...
class RPE1D(nn.Module):
    def __init__(self, num_heads, q_size, kv_size, with_rpe):
        super().__init__()
        self.q_size = q_size
    ...
    @staticmethod
    def _get_coord(l, device="cpu"):
        return torch.arange(l, device=device)

    def _get_relative_position_index(self, q_size, kv_size):
        device = self.relative_position_bias_table.device
        target_len = q_size + kv_size - 1
        coords_q = self._get_coord(q_size)
        coords_kv = self._get_coord(kv_size)

        rel_coords = coords_q[:, None] - coords_kv[None, :]
        rel_coords += kv_size - 1
        return rel_coords

    def forward(self, q_size, kv_size):
        if self.with_rpe:
            target_index = self._get_relative_position_index(q_size, kv_size)
            target_size = q_size + kv_size - 1
            rpb = self.relative_position_bias_table.view(
                1,
                (self.q_size + self.kv_size - 1),
                -1,
            ).permute(0, 2, 1)
            if target_size != rpb.size(2):
                rpb = F.interpolate(
                    rpb, size=target_size, mode="linear", align_corners=False
                )
            rpb = rpb.permute(0, 2, 1).squeeze(0)
            rpb = rpb[target_index].view(q_size, kv_size, -1)
            relative_position_bias = rpb.permute(
                2, 0, 1
            ).contiguous()  # nH, q_size, kv_size
        else:
            relative_position_bias = None
...
class MultiheadAttention(nn.Module):
    ...
    def __init__(
        ...
        if rpe_type == "2D":
            self.rpe = RPE2D(num_heads, q_window_size, kv_window_size, with_rpe)
        elif rpe_type == "1D":
            self.rpe = RPE1D(num_heads, q_size, kv_size, with_rpe)
        else:
            raise RuntimeError
    ...
    def forward(
        ...
        elif self.rpe_type == "1D":
            residual_attn = self.rpe(query.size(0), key.size(0))
